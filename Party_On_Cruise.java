...
0<= L[i] <=500 */
import java.util.*;
class Party_On_Cruise{
    public static int Max_Guest_Count(int t, int[] entering, int[] exitting)
    {
        int temp = 0;
        int[] count = new int[t];
        //temp = entering[0] - exitting[0];
        //count[0] = temp;
        for(int i=0;i<t;i++)
        {
            
            temp = (temp + entering[i]) - exitting[i];
            count[i] = temp;
        }
        int temp1 = count[0];
        for(int i=0;i<t;i++)
    ...
    public static void main(String args[])
    {
        ...
        for(int i=0;i<t;i++)
        {
            exitting[i] = sc.nextInt();
        }
        int max_guest = Max_Guest_Count(t,entering,exitting);
